const fs = require('fs');
const content = fs.readFileSync('C:/Users/buzzkill/.claude/projects/C--Users-buzzkill/01953930-1119-43d3-90e2-e7d532fbe0d2.jsonl', 'utf8');
...
// Find commands that return 'Bash'
console.log('Commands that parse to "Bash" (unrecognized):');
    ...
    if (entry.message?.content && Array.isArray(entry.message.content)) {
      for (const block of entry.message.content) {
        if (block.type === 'tool_use' && block.name === 'Bash') {
          ...
          if (result[0] === 'Bash') {
            console.log('UNRECOGNIZED:', cmd?.substring(0, 150));
          }
        }
      }
    }
