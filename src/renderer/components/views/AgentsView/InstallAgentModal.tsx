...
import { X, Download, Globe, FolderOpen, Bot, Wrench, Shield } from 'lucide-react';
...
export function InstallAgentModal({
  ...
  // Reset state when modal opens with a new agent
  useEffect(() => {
    if (isOpen) {
      setScope('user');
  ...
  const handleInstall = useCallback(async () => {
    ...
    onInstall(agent, scope, projectPath);
    onClose();
  }, [agent, scope, selectedProjectPath, onInstall, onClose]);
    ...
    >
      <FocusTrap>
        <div
          role="dialog"
          aria-modal="true"
          aria-labelledby="install-agent-modal-title"
        ...
        >
          ...
          <div className="flex items-center justify-between p-4 border-b border-surface-700">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2
                  id="install-agent-modal-title"
                  className="text-lg font-medium text-surface-100"
                >
                  Install Agent
                </h2>
                <p className="text-sm text-surface-400">{agent.name}</p>
              </div>
            </div>
            <button
              onClick={onClose}
